...
from utils.tools import plot_one_box, LoadStreams, LoadImages
CONF_THRESH = 0.50
IOU_THRESHOLD = 0.50
...
def infer_images_videos_rtsps(source):
    source_type = source.endswith('.txt') or source.lower().startswith('rtsp://')
    if source_type:
        print('loading RTSP ...')
        dataset = LoadStreams(source, img_size=640)
        for paths, img, im0s, vid_cap in dataset:
            # print(im0s)
            for path, im0 in zip(paths, im0s):
                handled, used_time = infer(yolov5_wrapper, im0)
        ...
        for paths, img, im0s, vid_cap in dataset:
            ...
            cv2.imshow(str(paths), handled)
            cv2.waitKey(10)

if __name__ == "__main__":
    # load custom plugins
    PLUGIN_LIBRARY = "buildx/libmyplugins.so"
    engine_file_path = "buildx/yolov5x.engine"
    ...
    try:
        # infer_images_videos_rtsps("stream.txt")
        infer_images_videos_rtsps("./inference/videos")  # inference path(images, videos, rtsp)
    finally:
